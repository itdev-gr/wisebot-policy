...
const Layout: React.FC<LayoutProps> = ({ children, lang, setLang, xp, level, completedIds, myHeroes = [] }) => {
  ...
  const getLockMessage = (path: string) => {
    ...
    switch (path) {
      case '/ebooks': return 'Διάβασε 2 ιστορίες από Ακαδημία';
      case '/factory': return 'Διάβασε 1 βιβλίο (με το κουίζ)';
      case '/cinema': return 'Δημιούργησε ήρωα & διάβασε 3 βιβλία';
      case '/3d-factory': return 'Διάβασε 5 βιβλία';
      case '/wise-friends': return 'Διάβασε βιβλία';
      default: return 'ΚΛΕΙΔΩΜΕΝΟ';
    }
  };
